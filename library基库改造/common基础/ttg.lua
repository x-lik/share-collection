...
function ttg.display(x, y, content, during, options)
    ...
    --- 漂浮文字利用器
    local reuse = Reuse("ttg")
    local tt = reuse:allocation(
        function(this)
            J.SetTextTagText(this, content, size * 0.023 / 10)
            J.SetTextTagColor(this, 255, 255, 255, math.floor(opacity))
            J.SetTextTagPos(this, x - vistring.len(content) * size * 0.5, y, zOffset)
            if (xOffset ~= 0 or yOffset ~= 0) then
                J.SetTextTagVelocity(this, xOffset, yOffset)
            end
            J.SetTextTagVisibility(this, true)
        end,
        function()
            local t = ttg.create()
            J.SetTextTagPermanent(t, false)
            return t
        end)
    ...
    time.setTimeout(during, function()
        ---@param this number
        reuse:recovery(tt, function(this)
            J.SetTextTagVisibility(this, false)
        end)
    end)
end